...
float get_reward ( node_t* n ){
	...
	reward = heuristic(n) + (float)((n->state).Points - parent_score);

	float discount = pow(0.99,n->depth);
   	// printf("reward: %lf", discount*reward);
...
bool applyAction(node_t* n, node_t** new_node, move_t action ){
    ...
    if(changed_dir){
		...
		(*new_node)->reward = get_reward(*new_node);		
		(*new_node)->acc_reward =  (*new_node)->reward + n->acc_reward;
...
int choose_best_action(float* best_action_score){
	...
	//breaking ties - insert indexes of top scores, choose one randomly
	if(counter==1){
		return best_action;
	} else {
